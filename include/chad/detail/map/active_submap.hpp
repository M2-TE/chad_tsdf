...
namespace chad::detail::map {
    ...
    private:
        ...
        void inline dda(
                ...
                const glm::aligned_dvec3& position,
                const glm::vec<3, T, glm::aligned_highp>& point_raw,
                double sdf_res,
                double sdf_trunc,
                double sdf_res_reciprocal) const {
            using glm_float_t = T;
            using glm_vec3f_t = glm::vec<3, glm_float_t, glm::aligned_highp>;
            glm_vec3f_t point = point_raw;
        ...
        void write_octree(const std::vector<glm::aligned_vec3>& points, const std::vector<glm::aligned_vec3>& normals, Pose pose, T sdf_res, T sdf_trunc) {
            ...
            for (/**/; points_it < std::cend(points); points_it++, normals_it++) {
                ...
                dda<T>(traversed_voxels, pose._position, point, sdf_res, sdf_trunc, sdf_res_reciprocal);
